...
def shoot_ray(odefn, event, rinit, rmax, theta0,  z0, dr, raytype):
    solver = 'DOP853'
    #dense_output = True
...
def objfn(theta, ode, rmax, z0, zm, dr, raytype):
    #function for rootfinder
    sol = shoot_ray(ode, hit_top, 0, rmax, theta, z0, dr, raytype)
    zsol = sol.y[1,-1]
    return zsol - zm

def get_ray_1guess(minfn, odefn, rmax, z0, zm, dr, raytype, boundguess): 
    lb, rb = get_bounds_1guess(boundguess, odefn, rmax, z0, zm, dr, raytype)
    ...
    odesol = shoot_ray(odefn, hit_top, 0, rmax, minsol.root, z0, dr, raytype)
    return odesol


def get_bounds_1guess(initguess, odefn, rmax, z0, zm, dr, raytype, odeparam = 'r', xtol = None, maxiter=None):
    ...
    dxi=1e-2
    dx = dxi
    zendintl = objfn(initguess, odefn, rmax, z0, zm, dr, raytype)
    dz = np.sign(zendintl)
    grad = np.sign(objfn(initguess+dx, odefn, rmax, z0, zm, dr, raytype) - zendintl)
    print(dz, grad)
    zendnew = zendintl
    inum = 0
    lastguess = initguess
    ...
    while dz == np.sign(zendnew) and newguess < np.pi/2 and newguess > 0:
        ...
        if (dz > 0 and grad < 0) or (dz < 0 and grad > 0):
            newguess += dx
        zendnew = objfn(newguess, odefn, rmax, z0, zm, dr, raytype)

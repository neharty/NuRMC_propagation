...
angs = np.linspace(np.arcsin(1/sf.ns(z0))-0.1, np.pi/2, num=300)
...
plt.vlines([np.arcsin(1/(sf.ns(z0))), np.arctan(sf.cont/np.sqrt(sf.cont**2*sf.ns(z0)**2 - 1))], min(min(zp), min(zs)), max(max(zp), max(zs)), colors=['g', 'k'])
print('npp',sf.npp(np.arctan(sf.cont/np.sqrt(sf.ns(z0)**2 - 1)), z0))

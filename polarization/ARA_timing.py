...
for j in range(len(phiarr)):
    ...
    ptime = time.time()
    podesol1, rb = sf.get_ray_1guess(sf.objfn, sf.podes, rmax, z0, zm, dr, 1)#, np.pi/2-np.arctan((-zm-z0)/rmax))
    if rb is not None:
    ...
    stime = time.time()
    sodesol1, rb = sf.get_ray_1guess(sf.objfn, sf.sodes, rmax, z0, zm, dr, 1)#, np.pi/2-np.arctan((-zm-z0)/rmax))
    if rb is not None:
